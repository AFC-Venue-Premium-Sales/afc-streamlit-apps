...
def run():
    ...
    def preprocess_manual(file):
        ...
        # Convert Event_Date to date only (remove time)
    ...
    def process_api_menu(api_df):
        ...
        for _, row in api_df.iterrows():
            ...
            eid = row.get('EventId')
            # Use KickOffEventStart as event date from API and convert to date
            raw_date = row.get("KickOffEventStart")
            event_date_api = pd.to_datetime(raw_date).date() if raw_date else None
    ...
    def map_event_id(row, event_map):
        # Use the manual file's Event_Date (date only) along with Location and Event for mapping.
