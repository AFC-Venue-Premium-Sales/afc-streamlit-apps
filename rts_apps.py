...
if not st.session_state["authenticated"]:
    ...
    **Please log in using AFC credentials to access the following modules:**
    ...
    # Sidebar filters for Sales Performance
    st.sidebar.header("Filter Data by Date and Time")
    date_range = st.sidebar.date_input("📅 Select Date Range", [], key="date_range_input")
    ...
    end_time = st.sidebar.time_input(
        "⏰ End Time",
        value=datetime.now().replace(hour=23, minute=59, second=59).time(),
        key="end_time_input"
    )
