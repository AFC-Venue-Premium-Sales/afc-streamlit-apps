...
def generate_kpis(filtered_data):
    ...
    # Calculate total revenue generated by the top exec
    if top_exec != "N/A":
        top_exec_revenue = filtered_data.loc[filtered_data['CreatedBy'] == top_exec, 'TotalPrice'].sum()
    else:
        top_exec_revenue = 0

    # Display revenue generated by the top exec
    st.metric("💷 Revenue Generated by Top Exec", f"£{top_exec_revenue:,.2f}")
...
def generate_trend_line(filtered_data):
    """Generate a trend line for sales trends."""
    st.write("### 📈 Sales Trends Over Time")
    filtered_data['CreatedDate'] = filtered_data['CreatedOn'].dt.date

    daily_revenue = (
        filtered_data.groupby('CreatedDate')['TotalPrice']
        .sum()
        .reset_index()
    )

    fig, ax = plt.subplots(figsize=(12, 6))
    ax.plot(daily_revenue['CreatedDate'], daily_revenue['TotalPrice'], marker='o', linestyle='-', label='Daily Revenue')
