...
targets_data = pd.DataFrame({
    ...
    "Year": [2024, 2024, 2024, 2025, 2025, 2025, 2025, 2025],
    "bgardiner": [155000, 155000,155000, 155000, 135000, 110000, 90000, 65000],
    "dcoppin":   [155000, 155000,155000, 155000, 135000, 110000, 90000, 65000],
    "jedwards":  [155000, 155000,155000, 155000, 135000, 110000, 90000, 65000],
    "MillieS":   [155000, 155000,155000, 155000, 135000, 110000, 90000, 65000],
    "millies":   [155000, 155000,155000, 155000, 135000, 110000, 90000, 65000],
    "dmontague": [155000, 155000,155000, 155000, 135000, 110000, 90000, 65000],
...
valid_sales_executives =  ["dcoppin", "MillieS", "millies", "bgardiner", "dmontague", "jedwards"]
...
def generate_scrolling_messages(data, budget_df, df_inventory):
    ...
    valid_executives = ["dcoppin", "MillieS", "millies", "bgardiner", "dmontague", "jedwards"]
...
def calculate_monthly_performance(data, targets_data, valid_sales_executives):
    """
    Returns a DataFrame indexed by (Month,Year) with two columns per exec:
    'Revenue' and '% of target'.
    """
    df = data.copy()
    df["CreatedOn"] = pd.to_datetime(df["CreatedOn"], errors="coerce", dayfirst=True)
    df["Month"] = df["CreatedOn"].dt.strftime("%B")
    df["Year"] = df["CreatedOn"].dt.year
    # sum up revenue
    rev = (
        df
        .groupby(["Month","Year","CreatedBy"])["Price"]
        .sum()
        .reset_index()
    )
    # melt targets to long form
    targets_long = (
        targets_data
        .reset_index()
        .melt(
    ...
    perf = pd.merge(
        rev, targets_long,
        on=["Month","Year","CreatedBy"],
        how="right"
    ).fillna(0)
    perf["% of target"] = (perf["Price"] / perf["Target"] * 100).round(0)
    # pivot so we get one row per (Month,Year)
    perf_rev = perf.pivot(index=["Month","Year"], columns="CreatedBy", values="Price")
    perf_pct = perf.pivot(index=["Month","Year"], columns="CreatedBy", values="% of target")
    # re-index in the same order as targets_data
    perf_rev = perf_rev.reindex(targets_data.index)
    perf_pct = perf_pct.reindex(targets_data.index)
    # build a MultiIndex-column DataFrame
    out = pd.DataFrame(index=targets_data.index)
    for exec in valid_sales_executives:
        out[(exec, "Revenue")]      = perf_rev[exec]
        out[(exec, "% of target")]  = perf_pct[exec]
...
def run_dashboard():
    ...
    def render_budget_progress_widget(data, valid_executives, title, start_date, end_date, targets_data):
        ...
        Includes a download button to export filtered sales data and a formatted
        'Monthly Performance' sheet with:
        • Month | Year | Target
        • then for each user: Revenue | % of target
        • merged usernames above their pair
        • red/amber/green conditional formatting on % columns
        """
        # 1️⃣ Parse dates & filter
        data["CreatedOn"] = pd.to_datetime(data["CreatedOn"], errors="coerce", dayfirst=True)
        executive_data = data[
        ...
        # 2️⃣ Compute sidebar summary
        total_revenue   = executive_data["Price"].sum()
        current_month   = start_date.strftime("%B")
        current_year    = start_date.year
        if (current_month, current_year) in targets_data.index:
            execs_in_targets = [e for e in valid_executives if e in targets_data.columns]
            total_target     = targets_data.loc[(current_month, current_year), execs_in_targets].sum()
        else:
            total_target     = 0
        progress_percentage = (total_revenue / total_target * 100) if total_target else 0

        # 3️⃣ Render HTML/CSS progress widget
        st.sidebar.markdown(
            f"""
            <style>
        ...
        # 4️⃣ Build & stream Excel report if data exists
        if executive_data.empty:
            return

        # 4a) Get performance pivot (Revenue & % of target)
        perf_df = calculate_monthly_performance(executive_data, targets_data, valid_executives)

        # 4b) Bring Month & Year into columns and insert Target
        perf = perf_df.reset_index()  # cols: Month, Year, (exec,Revenue), (exec,%)
        first_exec    = valid_executives[0]
        target_series = targets_data[first_exec].reindex(perf_df.index).values
        perf.insert(2, "Target", target_series)

        # 4c) Flatten all column names for Excel
        flat_cols = []
        for col in perf.columns:
            if isinstance(col, tuple):
                # tuple from perf_df, but after reset_index only Month,Year,Target and exec tuples remain
                flat_cols.append(f"{col[0]} {col[1]}")
            else:
                flat_cols.append(col)
        perf_flat = perf.copy()
        perf_flat.columns = flat_cols

        # 4d) Write to BytesIO Excel
        buffer = io.BytesIO()
        with pd.ExcelWriter(buffer, engine="xlsxwriter") as writer:
            # Sheet1: raw sales
            executive_data.to_excel(writer, sheet_name="Sales Data", index=False)

            # Sheet2: Monthly Performance
            sheet = "Monthly Performance"
            perf_flat.to_excel(
                writer,
                sheet_name=sheet,
                index=False,
                header=False,   # we add our own two header rows
                startrow=2      # data begins on row 3
            )

            wb = writer.book
            ws = writer.sheets[sheet]

            # a) Sub-headers on row 2
            sub_headers = ["Month", "Year", "Target"]
            for _ in valid_executives:
                sub_headers += ["Revenue", "% of target"]
            for col_idx, header in enumerate(sub_headers):
                ws.write(1, col_idx, header)

            # b) Merge & center exec names across their two columns on row 1
            ...
            for i, name in enumerate(valid_executives):
                first_col = 3 + i*2
                last_col  = first_col + 1
                ws.merge_range(0, first_col, 0, last_col, name, header_fmt)

            # c) Freeze the header rows
            ws.freeze_panes(2, 0)

            # d) Conditional formatting on each "% of target" column
            green = wb.add_format({'bg_color':'#C6EFCE','font_color':'#006100'})
            amber = wb.add_format({'bg_color':'#FFEB9C','font_color':'#9C6500'})
            red   = wb.add_format({'bg_color':'#FFC7CE','font_color':'#9C0006'})
            start_row = 2
            end_row   = start_row + len(perf_flat) - 1
            for i in range(len(valid_executives)):
                pct_col = 3 + i*2 + 1
                # ≥100%
                ws.conditional_format(start_row, pct_col, end_row, pct_col,
                                    {'type':'cell','criteria':'>=','value':100,'format':green})
                # 50–99.99%
                ws.conditional_format(start_row, pct_col, end_row, pct_col,
                                    {'type':'cell','criteria':'between',
                                    'minimum':50,'maximum':99.9999,'format':amber})
                # <50%
                ws.conditional_format(start_row, pct_col, end_row, pct_col,
                                    {'type':'cell','criteria':'<','value':50,'format':red})

        buffer.seek(0)
        st.sidebar.download_button(
            label="📥 Download Sales & Performance",
            data=buffer,
            file_name=f"sales_and_performance_{start_date.strftime('%b_%Y')}.xlsx",
